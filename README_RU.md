...
*Это форк оригинального репозитория [timoncool/prompt-warrior](https://github.com/timoncool/prompt-warrior), созданного для Claude Code. Данная версия полностью адаптирована для Hermes Agent с собственным коннектором к SQLite и обновлёнными ссылками.*

**v0.2**
Поправлено:
- Поправлена генерация аватарки
- Почищены коментарии и зависимости от "Claude Code"
Извесные проблемы:
- "Стиль работы — вызовы по ролям" неверно интерпритирует инструменты Hermes, будет исправлено в будущей версии
- "Файлы в работе — топ расширений" не обнаруживает расширения файлов, будет исправлено в будущей версии
...
Prompt Warrior (Hermes Edition) — агентский скилл, который читает локальную SQLite базу данных Hermes Agent (`state.db`) и превращает логи сессий в RPG-лист персонажа.
...
- **Наконец узнаете, кто вы.** Не «разработчик», а *Неистовый Командир Терминала
  ...
  Воин Hermes Agent верхом на Могучем Опусе.
...
**Ленивый вариант — одно сообщение.** Вставь это в Hermes Agent целиком — он сам всё
установит и прогонит:

```text
Установи скилл Prompt Warrior с гитхаба и сразу прогони его по полной.

Установка: https://github.com/PetrBachmanov/prompt-warrior-hermes.git в СВОЮ
директорию скиллов, папкой prompt-warrior (у Hermes Agent это ~/hermes/hermes-agent/skills/;
...
1. **Клонировать**
   ```bash
   git clone https://github.com/PetrBachmanov/prompt-warrior-hermes.git "Вставьте сюда путь к папке skills вашего Hermes Agent"
   ```

2. **Попросить Hermes Agent**
   ```
   /prompt-warrior
   ```
...
Под капотом: `scripts/analyze.py` читает локальную базу данных `state.db` Hermes Agent (только чтение), дедуплицирует ваши реплики, считает метрики и пишет `profile.json`
