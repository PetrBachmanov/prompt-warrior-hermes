...
Prompt Warrior (Hermes Edition) is an agent skill that reads your local Hermes Agent SQLite database (`state.db`) and turns your session logs into something between a psychological profile and an RPG character sheet: real numbers on a fixed scale, a title you didn't choose but absolutely deserve, and achievements you've been grinding for months without knowing it. Runs 100% locally on stdlib Python — nothing ever leaves your machine.
...
Under the hood: `scripts/analyze.py` reads the local SQLite database `state.db` of Hermes Agent (read-only), deduplicates your messages, computes metrics, and writes `profile.json`.
HTML or reads SVG.
