...
def collect_hermes(db_path, aux=None):
    if aux is None:
        aux = {}
    aux.update({
        "interruptions": 0, "assistant_msgs": 0, "thinking": 0, "images": 0,
        "tool_errors": 0, "prs": 0, "plan_modes": 0, "queued": 0,
        "tok_in": 0, "tok_out": 0, "tok_cache_read": 0, "tok_cache_new": 0,
        "pr_urls": set(), "double_texts": 0, "usage_seen": set(),
        "versions": set(), "branches": set(),
    ...
    # 1. Gather sessions stats
    cursor.execute("SELECT id, title, model, cwd, git_branch, input_tokens, output_tokens, cache_read_tokens, cache_write_tokens FROM sessions")
    for row in cursor.fetchall():
        sid, title, model, cwd, git_branch, tok_in, tok_out, tok_cache_read, tok_cache_new = row
        if title:
            aux["session_names"][sid] = title
        if model:
            aux["models"][model] = aux["models"].get(model, 0) + 1
        if cwd:
            aux["projects"][cwd] = aux["projects"].get(cwd, 0) + 1
        if git_branch:
            aux["branches"].add(git_branch)
            
        if tok_in: aux["tok_in"] += tok_in
        if tok_out: aux["tok_out"] += tok_out
        if tok_cache_read: aux["tok_cache_read"] += tok_cache_read
        if tok_cache_new: aux["tok_cache_new"] += tok_cache_new
        if tok_out:
            aux["session_tokens"][sid] = tok_out

    # 2. Gather user messages
    cursor.execute("SELECT content, role, timestamp, session_id, tool_calls FROM messages ORDER BY timestamp ASC")
    messages = []
    seen_text = set()
    prev_user = False
    
    for row in cursor.fetchall():
        content, role, ts, session_id, tool_calls = row
        if not content:
            continue
        ...
        try:
            if content.startswith('{') or content.startswith('['):
                ...
                if isinstance(parsed, list):
                    text = " ".join(b.get("text", "") for b in parsed if isinstance(b, dict) and b.get("type") == "text")
        ...
        if tool_calls:
            try:
                ...
                for call in calls:
                    ...
                    aux["tools"][tname] = aux["tools"].get(tname, 0) + 1
            except Exception:
                pass
        ...
        if role == "user":
            ...
            words = len(text.split())
            # Convert float/int timestamp to ISO string format "YYYY-MM-DD"
            import datetime
            ts_str = ""
            if ts:
                try:
                    ts_str = datetime.datetime.fromtimestamp(float(ts)).isoformat()
                except Exception:
                    ts_str = str(ts)
...
SOURCES = {
    ...
    "hermes": {
        "collect": lambda d, proj, aux=None: collect_hermes(d, aux),
        "default_dir": "C:/Users/pbach/AppData/Local/hermes/state.db",
    }
}
...
def main():
    ...
    ap.add_argument("--source", choices=sorted(SOURCES), default="hermes",
                    help="which harness logs to read (default: hermes)")
